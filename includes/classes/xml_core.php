...
*/
  class xml_core {
    ...
    function &insert_entry($tag, $close=false, $auto_close=true, $inner_content='') {
      $this->tag_index++;
      if(!$close) {
        $this->xml_array[$tag . $this->separator . $this->tag_index] = '<' . $tag . (($auto_close==true)?'>':' ');
        if( !$auto_close && !empty($inner_content) ) {
          $this->xml_array[$tag . $this->separator . $this->tag_index] .= $inner_content . '>';
        }
      } else {
        $this->xml_array[$tag . $this->separator . $this->tag_index] = '</' . $tag . '>';
      }
      return $this->xml_array[$tag . $this->separator . $this->tag_index];
    }
