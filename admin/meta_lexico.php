...
*/
  ...
  switch($action) {
    case 'setflag':
      ...
      tep_redirect(tep_href_link(FILENAME_META_LEXICO, tep_get_all_get_params(array('action','flag','tag_id')) ));
      break;
    ...
    case 'insert':
      ...
      tep_redirect(tep_href_link(FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) ));
      break;
    ...
    case 'update_multi':
      ...
      foreach ($_POST['tag_id'] as $key=>$value) {
        ...
        $g_db->perform(TABLE_META_LEXICO, $sql_data_array);
        //$g_db->perform(TABLE_META_LEXICO, $sql_data_array, 'update', "meta_lexico_key = '" . $g_db->filter($key) . "'");
      }
      tep_redirect(tep_href_link(FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) ));
      break;
    ...
    case 'generate':
      ...
      if($check_array = $g_db->fetch_array($check_query) ) {
        require(DIR_WS_CLASSES . $check_array['meta_types_class'] . '.php');
        $cMeta = new $check_array['meta_types_class'];
        $params = $cMeta->generate_lexico();
      }
      tep_redirect(tep_href_link(FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) . $params ));
      break;
...
?>
          ...
          <div class="formArea"><?php echo tep_draw_form('rl_confirm', FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) . 'action=delete_confirm_multi', 'post'); ?><table class="tabledata" cellspacing="1">
...
<?php 
    if( count($_POST['tag_id']) ) {
      echo '<a href="' . tep_href_link(FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) ) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a> ' . tep_image_submit('button_confirm.gif', IMAGE_CONFIRM);
    }
?>
              </td>
            </tr>
...
?>
          <div class="formArea"><?php echo tep_draw_form('rl', FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) . 'action=delete_multi', 'post'); ?><table class="tabledata" cellspacing="1">
            <tr>
              <td colspan="4" class="formButtons"><?php echo tep_image_submit('button_update.gif', TEXT_UPDATE, 'onclick="this.form.action=' . '\'' . tep_href_link(FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) . 'action=update_multi') . '\'' . '"') . ' ' . tep_image_submit('button_delete.gif', TEXT_DELETE, 'onclick="this.form.action=' . '\'' . tep_href_link(FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) . 'action=delete_multi') . '\'' . '"'); ?></td>
            </tr>
            <tr class="dataTableHeadingRow">
...
<?php
      ...
      while ($meta = $g_db->fetch_array($meta_query)) {
        ...
        echo '                      <tr class="' . $row_class . '">';
?>
              <td><?php echo tep_draw_checkbox_field('tag_id[' . $meta['meta_lexico_key'] . ']', ($bCheck?'on':''), $bCheck ); ?></td>
...
<?php
    if ($meta['meta_lexico_status'] == '1') {
      echo tep_image(DIR_WS_ICONS . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN) . '<a href="' . tep_href_link(FILENAME_META_LEXICO, 'action=setflag&flag=0&tag_id=' . $meta['meta_lexico_key'], 'NONSSL') . '">' . tep_image(DIR_WS_ICONS . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT) . '</a>';
    } else {
      echo '<a href="' . tep_href_link(FILENAME_META_LEXICO, 'action=setflag&flag=1&tag_id=' . $meta['meta_lexico_key'], 'NONSSL') . '">' . tep_image(DIR_WS_ICONS . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT) . '</a>' . tep_image(DIR_WS_ICONS . 'icon_status_red.png', IMAGE_ICON_STATUS_RED);
    }
?>
              </td>
            </tr>
...
?>
          ...
          <div class="splitLine">
            ...
            <div class="floatend"><?php echo $meta_split->display_links(tep_get_all_get_params(array('action', 'page'))); ?></div>
          </div>
<?php 
    }
?>
          ...
          <div class="formArea"><?php echo tep_draw_form('mz', FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) . 'action=generate', 'post'); ?><table border="0" cellspacing="1" cellpadding="3">
<?php
...
?>
          ...
          <div class="comboHeading">
            <div class="pageHeading"><h1><?php echo HEADING_TITLE3; ?></h1></div>
          </div>
          <div class="comboHeading">
            <div><?php echo TEXT_INFO_MAIN3; ?></div>
          </div>

          <div class="formArea"><?php echo tep_draw_form('mz', FILENAME_META_LEXICO, tep_get_all_get_params(array('action')) . 'action=insert', 'post'); ?><table border="0" cellspacing="1" cellpadding="3">
<?php
      echo '<td class="smallText"><b>Enter Phrase:</b></td>' . "\n";
      echo '<td>' . tep_draw_input_field('phrase', '', '', false, 'text', true) . '</td>' . "\n";
      echo '<td class="smallText"><b>Sort Order:</b></td>' . "\n";
      echo '<td>' . tep_draw_input_field('sort', '0', '', false, 'text', true) . '</td>' . "\n";
