...
define('MAX_SQL_ARRAY_SIZE', 5000);
  ...
  class dbase {
    ...
    function multi_insert($table, $data) {
      if( !is_array($data) || !count($data) || empty($table) ) return false;
      $columns = $data[0];
      if( !is_array($columns) || !count($columns) ) return false;
      $query = '';
      $query .= 'insert into ' . $table . ' (';
    ...
    function prepare_input($string, $sanitize = false) {
      if (is_string($string)) {
        if( $sanitize ) {
