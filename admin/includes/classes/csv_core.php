...
// - Converted mysql calls to tep_xx wrapper functions
...
*/
  ...
  /**
    ...
    */
    function core_csv($del=";", $quote='"', $lineend="\n") {
  ...
  /**
    ...
    */
    function queryImport($res) {
      global $g_db;
      $fc = $g_db->num_fields($res);
      if ($fc==0) return;
      for ($i=0; $i < $fc; $i++)
        $data[] = $g_db->field_name($res,$i);
      $this->addLine($data);
  ...
  /**
    ...
    */
    function queryExport($sql_raw, $name, $ext=".csv", $nameContainsExt=false) {
      ...
      $result_query = $g_db->query($sql_raw);
      $fc = $g_db->num_fields($result_query);

      if ($fc==0) return;
      if ($this->head) {
        for ($i=0; $i < $fc; $i++) {
          $data[$i]['Name'] = $g_db->field_name($result_query,$i);
          $data[$i]['Type'] = $g_db->field_type($result_query,$i);
      ...
      while ($row = $g_db->fetch_array($result_query)) {
        $i = 0;
        $fc = count($row);
