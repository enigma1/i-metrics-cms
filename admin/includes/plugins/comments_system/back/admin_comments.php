...
*/
  ...
  switch( $action ) {
    ...
    case 'delete_confirm':
      ...
      tep_redirect(tep_href_link($g_script, tep_get_all_get_params('action', 'cmID')));
      break;
    ...
    case 'edit_confirm':
      ...
      if( $error ) {
        tep_redirect(tep_href_link($g_script, tep_get_all_get_params('action') . 'action=edit'));
      }
...
<?php
  ...
  $options = $g_plugins->get_options('comments_system');
...
<?php
   if( !isset($options['collection_include']) || $options['collection_include'] ) {
...
?>
        <div class="maincell wider">
          ...
          <div class="comboHeading">
<?php
   if( !isset($options['text_include']) || $options['text_include'] ) {
...
?>
          <div class="splitLine">
            <div class="floater"><?php echo $comments_split->display_count(TEXT_DISPLAY_NUMBER_OF_ENTRIES); ?></div>
            <div class="floatend"><?php echo $comments_split->display_links(tep_get_all_get_params('page')); ?></div>
          </div>
          <div class="formArea"><?php echo tep_draw_form('comments_form', $g_script,'action=delete_all_confirm', 'post'); ?><table class="tabledata">
            <tr class="dataTableHeadingRow">
              <th class="calign"><?php echo '<a href="#mark" class="page_select" title="' . TEXT_PAGE_SELECT . '">' . tep_image(DIR_WS_ICONS . 'icon_tick.png', TEXT_PAGE_SELECT) . '</a>'; ?></th>
              <th><?php echo TABLE_HEADING_TITLE; ?></th>
              <th><?php echo TABLE_HEADING_TYPE; ?></th>
...
<?php
      ...
      while( $comments_array = $g_db->fetch_array($comments_query) ) {
        ...
        $sel_link = tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $comments_array['auto_id'] . '&action=edit');
        $inf_link = tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $comments_array['auto_id']);
        $comments_array['ip_address'] = $g_http->ip2s($comments_array['ip_address']);
...
?>
              <td class="calign"><?php echo tep_draw_checkbox_field('mark['.$comments_array['auto_id'].']', 1); ?></td>
...
<?php
      if( $comments_array['status_id'] == '1' ) {
        echo tep_image(DIR_WS_ICONS . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN) . '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'flag', 'gcID') . 'action=setflag&flag=0&gcID=' . $comments_array['comments_id']) . '">' . tep_image(DIR_WS_ICONS . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT) . '</a>';
      } else {
        echo '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'flag', 'gcID') . 'action=setflag&flag=1&gcID=' . $comments_array['comments_id']) . '">' . tep_image(DIR_WS_ICONS . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT) . '</a>' . tep_image(DIR_WS_ICONS . 'icon_status_red.png', IMAGE_ICON_STATUS_RED);
      }
?>
              </td>
              <td class="tinysep calign">
<?php
        if( $comments_array['read_id'] ) {
          echo '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $comments_array['auto_id']) . '">' . tep_image(DIR_WS_ICONS . 'icon_read.png', ICON_PREVIEW) . '</a>';
        } else {
          echo '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $comments_array['auto_id']) . '">' . tep_image(DIR_WS_ICONS . 'icon_unread.png', ICON_UNREAD) . '</a>';
        }
        echo '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $comments_array['auto_id'] . '&action=delete') . '">' . tep_image(DIR_WS_ICONS . 'icon_delete.png', TEXT_DELETE) . '</a>';
        echo '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $comments_array['auto_id'] . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'icon_edit.png', TEXT_EDIT) . '</a>';
...
<?php
    ...
    switch ($action) {
      case 'edit':
        ...
        $contents[] = array('form' => tep_draw_form('comment_edit', $g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $cmInfo->auto_id . '&action=edit_confirm') . tep_draw_hidden_field('auto_id', $cmInfo->auto_id));
        ...
        $contents[] = array('text' => TEXT_INFO_COMMENT . '<br />' . tep_draw_textarea_field('comments_body', $cmInfo->comments_body, '', 12));
        ...
        $buttons = array(
          '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $cmInfo->auto_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>',
          tep_image_submit('button_update.gif', IMAGE_UPDATE),
        );
        $contents[] = array(
          'class' => 'calign',
        ...
        break;
      case 'delete':
        if( isset($cmInfo) && is_object($cmInfo) ) {
          $heading[] = array('text' => '<b>' . sprintf(TEXT_HEADING_DELETE_COMMENT, $comments_array['title']) . '</b>');
          $contents[] = array('form' => tep_draw_form('form_comment', $g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $cmInfo->auto_id . '&action=delete_confirm') . tep_draw_hidden_field('auto_id', $cmInfo->auto_id));
          ...
          $contents[] = array('class' => 'calign', 'text' => tep_image_submit('button_delete.gif', IMAGE_DELETE) . '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $cmInfo->auto_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      ...
      default:
        if( isset($cmInfo) && is_object($cmInfo) ) {
          ...
          $contents[] = array('class' => 'infoBoxSection', 'text' => '<b>' . TEXT_INFO_COMMENT . '</b><br />' . $cmInfo->comments_body);
          if( !$cmInfo->read_id ) {
            $g_db->query("update " . TABLE_COMMENTS . " set read_id='1' where auto_id = '" . (int)$cmInfo->auto_id . "'");
          }
        } else { // create comment dummy info
          $heading[] = array('text' => '<b>' . EMPTY_GENERIC . '</b>');
          $contents[] = array('class' => 'calign', 'text' => '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'action=new_comment') . '">' . tep_image(DIR_WS_IMAGES . 'invalid_entry.png', IMAGE_NEW) . '</a>');
