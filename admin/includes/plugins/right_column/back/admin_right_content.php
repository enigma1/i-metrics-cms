...
//----------------------------------------------------------------------------
// Admin: Right Column Control for all content types and text pages
//----------------------------------------------------------------------------
// I-Metrics CMS
//----------------------------------------------------------------------------
...
*/
  ...
  switch( $action ) {
    case 'setflag':
      $g_db->query("update " . TABLE_RIGHT_TO_CONTENT . " set status_id = '" . (int)$_GET['flag'] . "' where auto_id = '" . (int)$cmID . "'");
      tep_redirect(tep_href_link($g_script, tep_get_all_get_params('action', 'flag') ));
      break;
    case 'edit_confirm':
      $check_query = $g_db->query("select count(*) as total from " . TABLE_RIGHT_TO_CONTENT . " where auto_id = '" . (int)$cmID . "'");
      $check_array = $g_db->fetch_array($check_query);
      if( !$check_array['total'] ) {
        $messageStack->add_session(ERROR_CONTENT_INVALID);
        tep_redirect(tep_href_link($g_script, tep_get_all_get_params('action', 'type_id', 'cmID') ));
      }
      $sql_data_array = array(
        'content_id' => (int)$_POST['content_id'],
        'content_type' => $g_db->prepare_input($_POST['content_type']),
        'content_name' => $g_db->prepare_input($_POST['content_name']),
        'content_text' => $g_db->prepare_input($_POST['content_text']),
        'sort_id' => (int)$_POST['sort_id'],
        'status_id' => isset($_POST['status_id'])?1:0
      );
      $g_db->perform(TABLE_RIGHT_TO_CONTENT, $sql_data_array, 'update', "auto_id='" . (int)$cmID . "'");

      $messageStack->add_session(SUCCESS_INSERT_ASSIGNED, 'success');
      tep_redirect(tep_href_link($g_script, tep_get_all_get_params('action', 'type_id', 'cmID') . 'cmID=' . $cmID));
      break;
    case 'delete_confirm':
      if( isset($_POST['auto_id']) && !empty($_POST['auto_id']) ) {
        $auto_id = (int)$_POST['auto_id'];
        $g_db->query("delete from " . TABLE_RIGHT_TO_CONTENT . " where auto_id = '" . (int)$auto_id . "'");
    ...
    case 'delete_all_confirm':
      ...
      $g_db->query("delete from " . TABLE_RIGHT_TO_CONTENT . " where auto_id in ('" . implode("','", $tmp_array) . "')");
    ...
    case 'assign':
      ...
      if( isset($_POST['mark']) && is_array($_POST['mark']) && count($_POST['mark']) ) {
        foreach ($_POST['mark'] as $key=>$val) {
          ...
          $g_db->perform(TABLE_RIGHT_TO_CONTENT, $sql_data_array);
        }
      }
      $messageStack->add_session(SUCCESS_INSERT_ASSIGNED, 'success');
...
<?php
  ...
  //$plugin = $g_plugins->get('right_column');
...
<?php
      ...
      while( $list_array = $g_db->fetch_array($list_query) ) {
        ...
        $check_query = $g_db->query("select count(*) as total from " . TABLE_RIGHT_TO_CONTENT . " where content_id = '" . (int)$list_array['abstract_zone_id'] . "' and content_type='2'");
        $check_array = $g_db->fetch_array($check_query);
...
?>
              <td class="calign"><?php echo tep_draw_checkbox_field('mark['.$list_array['abstract_zone_id'].']', 1, false); ?></td>
...
<?php
      ...
      while( $list_array = $g_db->fetch_array($list_query) ) {
        ...
        $check_query = $g_db->query("select count(*) as total from " . TABLE_RIGHT_TO_CONTENT . " where content_id = '" . (int)$list_array['gtext_id'] . "' and content_type='1'");
        $check_array = $g_db->fetch_array($check_query);
        ...
        echo '          <tr class="' . $row_class . '">';
?>
              <td class="calign"><?php echo tep_draw_checkbox_field('mark['.$list_array['gtext_id'].']', 1, false); ?></td>
...
<?php
    ...
    $content_query_raw = "select auto_id, content_name from " . TABLE_RIGHT_TO_CONTENT . " where auto_id in ('" . implode("','", $tmp_array) . "')";
...
<?php
    $content_query_raw = "select auto_id, content_id, content_type, content_name, content_text, status_id from " . TABLE_RIGHT_TO_CONTENT . " "  . $sort_by . "";
...
?>
          ...
          <div class="formArea"><?php echo tep_draw_form('content_form', $g_script, 'action=delete_all', 'post'); ?><table class="tabledata">
            <tr class="dataTableHeadingRow">
              ...
              <th><?php echo TABLE_HEADING_BOX_TITLE; ?></th>
...
<?php
      ...
      while( $content_array = $g_db->fetch_array($content_query) ) {
        ...
        if( !empty($cmID) && $cmID == $content_array['auto_id'] ) {
          $cmInfo = new objectInfo($content_array);
...
<?php
      if( $content_array['status_id'] == '1' ) {
        echo tep_image(DIR_WS_ICONS . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN) . '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'flag', 'gcID') . 'action=setflag&flag=0&gcID=' . $content_array['content_id']) . '">' . tep_image(DIR_WS_ICONS . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT) . '</a>';
      } else {
        echo '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'flag', 'gcID') . 'action=setflag&flag=1&gcID=' . $content_array['content_id']) . '">' . tep_image(DIR_WS_ICONS . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT) . '</a>' . tep_image(DIR_WS_ICONS . 'icon_status_red.png', IMAGE_ICON_STATUS_RED);
      }
?>
              </td>
              <td class="tinysep calign">
<?php
        echo '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $content_array['auto_id'] . '&action=delete') . '">' . tep_image(DIR_WS_ICONS . 'icon_delete.png', TEXT_DELETE) . '</a>';
        echo '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $content_array['auto_id'] . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'icon_edit.png', TEXT_EDIT) . '</a>';
        if (isset($cmInfo) && is_object($cmInfo) && ($content_array['auto_id'] == $cmInfo->auto_id)) { 
          echo tep_image(DIR_WS_ICONS . 'icon_arrow_right.png', TEXT_SELECTED); 
        } else { 
          echo '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $content_array['auto_id']) . '">' . tep_image(DIR_WS_ICONS . 'icon_info.png', IMAGE_ICON_INFO) . '</a>'; 
        }
?>
              </td>
            </tr>
...
<?php
    ...
    switch ($action) {
      case 'edit':
        $content_query = $g_db->query("select auto_id, content_id, content_name, content_text, content_type, sort_id, status_id from " . TABLE_RIGHT_TO_CONTENT . " where auto_id = '" . (int)$cmID . "'");
        ...
        $contents[] = array('form' => tep_draw_form('content_edit', $g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $content_array['auto_id'] . '&action=edit_confirm') . tep_draw_hidden_field('auto_id', $content_array['auto_id']));
        ...
        $contents[] = array('text' => TEXT_INFO_NAME . '<br />' . tep_draw_input_field('content_name', $content_array['content_name']));
        $contents[] = array('text' => TEXT_INFO_TEXT . '<br />' . tep_draw_textarea_field('content_text', $content_array['content_text'], '', 12));
        ...
        $buttons = array(
          '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $content_array['auto_id']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>',
          tep_image_submit('button_update.gif', IMAGE_UPDATE),
        );
        $contents[] = array(
          'class' => 'calign',
      ...
      case 'delete':
        if( $rows > 0 && isset($cmInfo) && is_object($cmInfo) ) {
          $content_query = $g_db->query("select auto_id, content_name from " . TABLE_RIGHT_TO_CONTENT . " where auto_id = '" . (int)$cmID . "'");
          $content_array = $g_db->fetch_array($content_query);
          $heading[] = array('text' => '<b>' . sprintf(TEXT_HEADING_DELETE_CONTENT, $content_array['content_name']) . '</b>');
          $contents[] = array('form' => tep_draw_form('form_content', $g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $cmID . '&action=delete_confirm') . tep_draw_hidden_field('auto_id', $content_array['auto_id']));
      ...
      default:
        if( $rows > 0 && isset($cmInfo) && is_object($cmInfo) ) {
          $heading[] = array('text' => '<b>' . $cmInfo->title . '</b>');
          $buttons = array(
            '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $cmInfo->auto_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>',
            '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'cmID=' . $cmInfo->auto_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a>',
          );
          $contents[] = array(
            'class' => 'calign', 
            'text' => implode('', $buttons)
          );
          $contents[] = array('text' => TEXT_INFO_NAME . '<br />' . $cmInfo->content_name);
          $contents[] = array('text' => TEXT_INFO_TEXT . '<br />' . $cmInfo->content_text);
        } else { // create content dummy info
          $heading[] = array('text' => '<b>' . EMPTY_GENERIC . '</b>');
          $contents[] = array('class' => 'calign', 'text' => '<a href="' . tep_href_link($g_script, tep_get_all_get_params('action', 'cmID') . 'action=new_content') . '">' . tep_image(DIR_WS_IMAGES . 'invalid_entry.png', IMAGE_NEW) . '</a>');
