...
?>
        <div class="maincell" style="width: 100%;">
          <div class="comboHeading">
            <div class="pageHeading"><h1><?php echo HEADING_META_TYPES_ADD; ?></h1></div>
          </div>
          <div class="comboHeading">
          ...
          <div class="formArea"><?php echo tep_draw_form("add_field", basename($PHP_SELF), 'action=add', 'post'); ?><table class="tabledata" cellspacing="1">
            <tr class="dataTableHeadingRow">
              ...
              <th><?php echo TABLE_HEADING_META_LINKAGE; ?></th>
            </tr>
            <tr>
        ...
        <div class="maincell" style="width: 100%">
          <div class="comboHeading">
            <div class="pageHeading"><h1><?php echo HEADING_META_TYPES_UPDATE; ?></h1></div>
          </div>
          <div class="comboHeading">
            <div><?php echo TEXT_INFO_UPDATE; ?></div>
          </div>
          <div class="formArea"><?php echo tep_draw_form('meta_types', basename($PHP_SELF),'action=update', 'post'); ?><table class="tabledata" cellspacing="1">
            <tr class="dataTableHeadingRow">
              <th><?php echo '<a href="javascript:void(0)" onclick="copy_checkboxes(document.meta_types,\'mark\')" title="' . TEXT_PAGE_SELECT . '" class="menuBoxHeadingLink">' . tep_image(DIR_WS_ICONS . 'icon_tick.png', TEXT_PAGE_SELECT) . '</a>'; ?></td>
              <th><?php echo TABLE_HEADING_META_NAME; ?></th>
              <th><?php echo TABLE_HEADING_META_CLASS; ?></th>
              <th><?php echo TABLE_HEADING_SORT_ORDER; ?></th>
              <th><?php echo TABLE_HEADING_META_LINKAGE; ?></th>
...
?>
            <tr>
              <td><?php echo tep_draw_checkbox_field('mark['.$meta_types['meta_types_id'].']', 1) ?></td>
              <td><?php echo tep_draw_input_field('name[' . $meta_types['meta_types_id'] . ']', $meta_types['meta_types_name']); ?></td>
              <td><?php echo tep_draw_input_field('class[' . $meta_types['meta_types_id'] . ']', $meta_types['meta_types_class']) . '.php'; ?></td>
              <td><?php echo tep_draw_input_field('sort[' . $meta_types['meta_types_id'] . ']', $meta_types['sort_order'], 'size=3'); ?></td>
              <td><?php echo tep_draw_input_field('linkage['.$meta_types['meta_types_id'] . ']', $meta_types['meta_types_linkage'], 'size=3'); ?></td>
              <td class="tinysep calign">
<?php
    if ($meta_types['meta_types_status'] == '1') {
      echo tep_image(DIR_WS_ICONS . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN) . '<a href="' . tep_href_link(basename($PHP_SELF), 'action=setflag&flag=0&id=' . $meta_types['meta_types_id'], 'NONSSL') . '">' . tep_image(DIR_WS_ICONS . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT) . '</a>';
    } else {
      echo '<a href="' . tep_href_link(basename($PHP_SELF), 'action=setflag&flag=1&id=' . $meta_types['meta_types_id'], 'NONSSL') . '">' . tep_image(DIR_WS_ICONS . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT) . '</a>' . tep_image(DIR_WS_ICONS . 'icon_status_red.png', IMAGE_ICON_STATUS_RED);
    }
?>
              </td>
            </tr>
...
?>
            <tr>
              <td colspan="9" class="formButtons"><?php echo tep_image_submit('button_update.gif', IMAGE_UPDATE, 'class="dflt" name="update"') . tep_image_submit('button_delete.gif', IMAGE_DELETE, 'class="dflt" name="remove"') ?></td>
            </tr>
          </table></form></div>
        </div>
